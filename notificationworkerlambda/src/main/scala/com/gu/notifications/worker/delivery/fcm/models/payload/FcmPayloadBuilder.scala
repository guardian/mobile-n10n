...
object FcmPayloadBuilder {
  ...
  private def toPlatformLink(link: Link): PlatformUri = link match {
    case Link.Internal(contentApiId, _, _, Some(blockId)) => PlatformUri(s"x-gu:///items/$contentApiId?page=with:block-$blockId#$blockId", "item")
  ...
  private def toAndroidLink(link: Link) = link match {
    case Link.Internal(contentApiId, _, _, Some(blockId)) => new URI(s"x-gu://www.guardian.co.uk/$contentApiId?page=with:block-$blockId#$blockId")
