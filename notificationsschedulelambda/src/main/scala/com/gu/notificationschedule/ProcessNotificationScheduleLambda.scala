...
import com.gu.notificationschedule.dynamo.NotificationSchedulePersistence
...
class ProcessNotificationScheduleLambda(config: NotificationScheduleConfig, cloudWatch: CloudWatch, notificationSchedulePersistence: NotificationSchedulePersistence) {
  def this(config: NotificationScheduleConfig, lambdaName: String) = this(config, new CloudWatchImpl(config.stage, lambdaName, AmazonCloudWatchAsyncClientBuilder.defaultClient()), new NotificationSchedulePersistence(config, AmazonDynamoDBAsyncClientBuilder.defaultClient()) )
