...
object AggregationCounts {
  ...
  def aggregate(triedAggregationResults: Seq[Try[AggregationCounts]]): S3ResultCounts = {
    ...
    triedAggregationResults.foldLeft(S3ResultCounts(0, 0, AggregationCounts(0, 0))) {
      case (s3Result, Success(aggregationResult @ AggregationCounts(_, 0))) =>
        s3Result.copy(success = s3Result.success + 1, aggregationResults = aggregationResult.combine(s3Result.aggregationResults))
      case (s3Result, Success(aggregationResult)) =>
        s3Result.copy(failure = s3Result.failure + 1, aggregationResults = aggregationResult.combine(s3Result.aggregationResults))
      case (s3Result, Failure(NonFatal(exception))) =>
