  ...
  def pushToEventbus(payload: LiveActivityPayload)(implicit ec: ExecutionContext): Future[Unit] = {
    ...
    if (jsonDetail.isEmpty || jsonDetail == "{}") {
      ...
      result match {
        case Success(_) => {
          logger.info(
            s"Eventbus pusher: Successfully processed live activity event with id ${payload.id}"
          )
          Future.successful(())
        }
        case Failure(e) => {
          logger.error(
            s"Eventbus pusher: Failed to publish live activity event with id ${payload.id}: ${e.getMessage}"
          )
          Future.failed(e)
        }
      }
